...
from libmata.nfa.nfa cimport Symbol, State, StateSet, CDelta, CRun, CTrans, CNfa, CMove, CEPSILON
...
cdef class Nfa:
    ...
    def trim_with_state_map(self):
        ...
        """
        cdef umap[State, State] state_map
...
def concatenate_with_result_state_maps(Nfa lhs, Nfa rhs, use_epsilon: bool = False) \
    ...
    """
    result = Nfa()
    cdef umap[State, State] c_lhs_map
    cdef umap[State, State] c_rhs_map
    mata_nfa.c_concatenate(result.thisptr.get(), dereference(lhs.thisptr.get()), dereference(rhs.thisptr.get()),
