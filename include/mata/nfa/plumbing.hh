...
/**
 * @brief Same as @c Nfa::get_words_lazy(), but as a free function returning a heap-allocated wrapper instead of a
 *  @c std::generator.
 *
 * Exists for callers that cannot drive a C++23 coroutine directly — namely the Python bindings, generated by
 * Cython, which has no notion of C++23 coroutines/sentinels. Prefer @c Nfa::get_words_lazy() everywhere else.
 *
 ...
 */
inline utils::LazyWordGenerator*
	get_words_lazy_ptr(const Nfa& aut, const size_t max_length = std::numeric_limits<size_t>::max()) {
	return new utils::LazyWordGenerator(aut.get_words_lazy(max_length));
}
