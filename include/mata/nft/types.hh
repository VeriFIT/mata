...
class Levels : std::vector<Level> {
  ...
  public:
	...
	class Ordering {
	  public:
		...
		/**
		 * @brief Ordering for levels in NFTs where lower levels precede higher levels, except for 0 which is the
		 * highest level.
		 *
		 * That is, levels are ordered as follows: 1 < 2 < ... < num_of_levels-1 < 0.
		 * This ordering is used when handling intermediate states (with non-zero levels) in NFTs for determining the
		 * next lowest level of the next state.
		 */
	...
	// explicit Levels(std::initializer_list<Level> levels): super{ levels }, num_of_levels{
	// num_of_levels_of(levels).value_or(DEFAULT_NUM_OF_LEVELS) } {}
	explicit Levels(const std::vector<Level>& levels)
		: super{levels},
		  num_of_levels{num_of_levels_of(levels).value_or(DEFAULT_NUM_OF_LEVELS)} {}
	explicit Levels(std::vector<Level>&& levels) : super{std::move(levels)} {
		num_of_levels = num_of_levels_of(*this).value_or(DEFAULT_NUM_OF_LEVELS);
	}
	explicit Levels(const size_t num_of_levels, std::vector<Level> levels = {})
		: super{std::move(levels)},
	 ...
	 *
	 * After assignment, @c num_of_levels is set to the minimal number of levels that can accommodate all levels in @p
	 * other.
	 *
	...
	using
		// Member types.
		super::value_type,
		super::allocator_type, super::size_type, super::difference_type, super::reference, super::const_reference,
		super::pointer, super::const_pointer, super::iterator, super::const_iterator, super::reverse_iterator,
		...
		super::cend, super::rbegin, super::crbegin, super::rend, super::crend,
		// - Capacity.
	...
	bool operator==(const std::vector<Level>& other) const {
		return static_cast<const std::vector<Level>&>(*this) == other;
	}
	 ...
	 */
	template <StatesContainerIterable States> std::vector<StateSet> map_levels_to(const States& states) const {
	 ...
	 *
	 * "Minimal level" is defined as the level with the lowest numerical value, i.e., `0 < 1 < 2 < ... <
	 * num_of_levels-1`. "Minimal" often relates to the current states ("What is the current state with minimal level?")
	 */
	std::optional<Level>
		get_minimal_level_of(const StateSet& states, Ordering::Compare levels_ordering = Ordering::Minimal) const;
  ...
  private:
	bool check_levels_in_range_() const {
		return std::ranges::all_of(*this, [&](const Level level) { return level < num_of_levels; });
	}
};
