...
#define PRINT_VERBOSE_LVL(lvl, title, x)                                                                               \
	{                                                                                                                  \
		if (mata::LOG_VERBOSITY >= (lvl)) { std::cerr << (title) << ": " << x << "\n"; }                               \
	}
...
#define DEBUG_PRINT(x)                                                                                                 \
	{ PRINT_VERBOSE_LVL(2, "debug", x); }
#define DEBUG_PRINT_LN(x)                                                                                              \
	{ PRINT_VERBOSE_LVL_LN(2, "debug", x); }
#define DEBUG_VM_HIGH_PRINT(x)                                                                                         \
	{ PRINT_VERBOSE_LVL(3, "debug VM", x); }
#define DEBUG_VM_HIGH_PRINT_LN(x)                                                                                      \
	{ PRINT_VERBOSE_LVL_LN(3, "debug VM", x); }
#define DEBUG_VM_LOW_PRINT(x)                                                                                          \
	{ PRINT_VERBOSE_LVL(4, "debug VM", x); }
#define DEBUG_VM_LOW_PRINT_LN(x)                                                                                       \
	{ PRINT_VERBOSE_LVL_LN(4, "debug VM", x); }
#define WARN_PRINT(x)                                                                                                  \
	{ PRINT_VERBOSE_LVL(1, "warning", x); }
...
template <typename T, typename Predicate> auto enumerate(const std::vector<T>& vec, Predicate pred) {
	return std::views::iota(size_t{0}, vec.size()) | std::views::filter([&](const size_t i) { return pred(vec[i]); }) |
		   std::views::transform([&](const size_t i) { return std::pair(i, vec[i]); });
}

template <typename T, typename Predicate> auto filter_indices(const std::vector<T>& vec, Predicate pred) {
	return std::views::iota(size_t{0}, vec.size()) | std::views::filter([&](const size_t i) { return pred(vec[i]); });
}
...
template <class T> bool are_disjoint(const std::set<T>& lhs, const std::set<T>& rhs) { // {{{
...
template <class Vector> void inline reserve_on_insert(Vector& vec, size_t needed_capacity = 0, size_t extension = 32) {
	// return; //Try this to see the effect of calling this. It should not affect functionality.
	if (vec.capacity() < extension) // if the size is already large enough, leave it to the default doubling strategy.
									// This if seems to make a barely noticeable difference :).
	{
		if (vec.capacity() < std::max(vec.size() + 1, needed_capacity)) { vec.reserve(vec.size() + extension); }
	}
}

// This function reindexes vector, that is, the content of each index i will be moved to the index renaming[i].
//  It might be useful in revert and trim, but so far it is useless. It was hard to get right, so I am reluctant to
...
template <class Vector, typename Fun> void filter_indexes(Vector& vec, const Fun&& is_staying) {
	...
	for (size_t i = 0, size = vec.size(); i < size; ++i) {
		if (is_staying(i)) {
			if (i != last) { vec[last] = std::move(vec[i]); }
			last++;
		}
	}
	vec.reserve(last);
