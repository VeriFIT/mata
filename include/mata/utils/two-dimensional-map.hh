 ...
 */
template <typename T, bool TrackInverted = true, size_t MaxMatrixSize = 50'000'000> class TwoDimensionalMap {
  ...
  public:
	...
	T get(const T first, const T second) const {
		if (!is_large_) { return matrix_storage_[first][second]; }

		auto it = vec_map_storage_[first].find(second);
		if (it == vec_map_storage_[first].end()) { return std::numeric_limits<T>::max(); }
		return it->second;
	}
