 ...
 */
Cntnfa create_single_word_nfa(const std::vector<Symbol>& word);
 ...
 *  created.
 */
Cntnfa create_single_word_nfa(const std::vector<std::string>& word, Alphabet* alphabet = nullptr);

/**
 * Create automaton accepting only epsilon string.
 */
Cntnfa create_empty_string_nfa();
 ...
 */
Cntnfa create_sigma_star_nfa(Alphabet* alphabet = new OnTheFlyAlphabet{});

/** Construct an automaton with counters using ParsedSection */
Cntnfa construct_counter_nfa(const mata::parser::ParsedSection& parsec, Alphabet* alphabet, NameStateMap* state_map = nullptr);

/** Loads an automaton from Parsed object */
// TODO this function should the same thing as the one taking IntermediateAut or be deleted
Cntnfa construct(const mata::parser::ParsedSection& parsec, Alphabet* alphabet, NameStateMap* state_map = nullptr);

/** Loads an automaton from Parsed object */
Cntnfa construct(const mata::IntermediateAut& inter_aut, Alphabet* alphabet, NameStateMap* state_map = nullptr);
/** Loads an automaton from Parsed object; version for python binding */
void construct(
    Cntnfa* result, const mata::IntermediateAut& inter_aut, Alphabet* alphabet, NameStateMap* state_map = nullptr
);

template<class ParsedObject>
Cntnfa construct(const ParsedObject& parsed, Alphabet* alphabet = nullptr,
 ...
 */
Cntnfa parse_from_mata(std::istream& nfa_stream);
 ...
 * @throws std::runtime_error Parsing of NFA fails.
 */
Cntnfa parse_from_mata(const std::string& nfa_in_mata);
