...
#define RUN_EXAMPLE(smaller, bigger) do{\
    ParameterMap params = {{"algorithm", ALGO}};\
    if (is_included(smaller, bigger, nullptr, nullptr, params)) {\
        std::cout << "Language of the first automaton is included in the language of the second automaton." << std::endl;\
    } else {\
        std::cout << "Language of the first automaton is not included in the language of the second automaton." << std::endl;\
    }\
    std::cout << "\n\n";\
} while(0);
