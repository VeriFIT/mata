...
Nft reduce_size_by_simulation(const Nft& aut, StateRenaming& state_renaming) {
	...
	const auto sim_relation =
		algorithms::compute_relation(aut, ParameterMap{{"relation", "simulation"}, {"direction", "forward"}});

	auto sim_relation_symmetric = sim_relation;
	sim_relation_symmetric.restrict_to_symmetric();

	// for State q, quot_proj[q] should be the representative state representing the symmetric class of states in
	// simulation
	...
	for (State q = 0; q < num_of_states; ++q) {
		...
		if (quot_proj[q] == q) {
			...
			for (const auto& q_trans : aut.delta.state_post(q)) {
				...
				// get the class states of those representatives that are not simulated by another representative in
				...
				for (const State s : representatives_of_states_to) {
					...
					for (const State p : representatives_of_states_to) {
						if (s != p && sim_relation.get(s, p)) { // if p (different from s) simulates s
							...
							// as p simulates s, the transition from q to s is not important to keep, as it is subsumed
							// in transition from q to p
							break;
						}
					}
				...
				// add the transition 'q_class_state-q_trans.symbol->representatives_class_states' at the end of
				// transition list of transitions starting from q_class_state as the q_trans.symbol should be the
				// largest symbol we saw (as we iterate trough getTransitionsFromState(q) which is ordered)
				result.delta.mutable_state_post(q_class_state)
					.insert(SymbolPost(q_trans.symbol, representatives_class_states));
			}
...
[[maybe_unused]] const StateSet* epsilon_targets(const Nft& aut, const State state) {
	...
	if (state_post.empty() || state_post.back().symbol != nft::EPSILON) { return nullptr; }
...
bool is_in_lang_by_levels_repeat_symbol(const Nft& aut, const std::vector<Word>& level_words, const bool match_prefix) {
	...
	auto are_all_track_words_read = [&](const std::vector<Word::const_iterator>& word_begins) {
		for (Level i{0}; i < aut.levels.num_of_levels; ++i) {
	...
	if (aut.final.intersects_with(aut.initial) && words_match(track_words_begins)) { return true; }

	using StateWordBeginsPair = std::pair<State, std::vector<Word::const_iterator>>;
	std::deque<StateWordBeginsPair> worklist{};
	for (const State state : aut.initial) { worklist.emplace_back(state, track_words_begins); }
	while (!worklist.empty()) {
		const auto [state, words_its]{std::move(worklist.front())};
		worklist.pop_front();
		const Level level = aut.levels[state];
		const StatePost& state_post{aut.delta[state]};
		...
		auto symbol_post_it{state_post.find(nft::EPSILON)};
		if (symbol_post_it != state_post_end) {
			for (State target : symbol_post_it->targets) {
				if (aut.levels[target] == 0 && aut.final.contains(target) && words_match(words_its)) { return true; }
		...
		auto handle_symbol_it{[&] {
			for (State target : symbol_post_it->targets) {
				...
				const Level level_target{aut.levels[target]};
				auto next_words_its{words_its};
				if (level == 0 && level_target == 0) {
					for (Level level_loop{level}; level_loop < aut.levels.num_of_levels; ++level_loop) {
					...
					for (Level level_loop{level}; level_loop != level_target;
						 level_loop = aut.levels.next_level_after(level_loop)) {
						if (next_words_its[level_loop] == track_words_ends[level_loop] ||
							not symbols_match(symbol_post_it->symbol, *next_words_its[level_loop])) {
							jump_failed = true;
							break;
				...
				if (aut.levels[target] == 0 && aut.final.contains(target) && words_match(next_words_its)) {
					return true;
				}
		...
		if (*word_symbol_it == DONT_CARE) {
			...
			for (; symbol_post_it != state_post_end; ++symbol_post_it) {
				if (symbol_post_it->symbol == nft::EPSILON) { continue; }
		...
		if (*word_symbol_it != nft::EPSILON && symbol_post_it != state_post_end && handle_symbol_it()) { return true; }
...
bool mata::nft::has_epsilon_cycle(const Nft& nft) {
	...
	for (State source{0}; source < num_of_states; ++source) {
		...
		if (num_of_epsilon_predecessors.empty()) { num_of_epsilon_predecessors.assign(num_of_states, 0); }
		epsilon_sources.push_back(source);
		for (const State target : *targets) { ++num_of_epsilon_predecessors[target]; }
	}
	...
	for (State state{0}; state < num_of_states; ++state) {
		if (num_of_epsilon_predecessors[state] != 0) { ++num_of_states_on_epsilon_transitions; }
	}
	...
	while (!peelable.empty()) {
		...
		if (const StateSet* const targets{epsilon_targets(nft, state)}; targets != nullptr) {
			for (const State target : *targets) {
				if (--num_of_epsilon_predecessors[target] == 0) { peelable.push_back(target); }
			}
		}
	}
				...
				for (const std::vector<State>& safe_epsilon_run : state_safe_epsilon_runs) {
					...
					if (const auto eps_move_it_s{post_s.find(epsilon)}; eps_move_it_s != post_s.end()) {
						if (cur_level != DEFAULT_LEVEL && (aut.delta[state_s].size() != 1)) {
							// for levels > DEFAULT_LEVEL we do not want to have transitions that are not with empty
							// symbol
							continue;
						}
						if (cur_level != aut.levels.num_of_levels - 1 && eps_move_it_s->targets.size() > 1) {
							// if we are not at the last level (next level will be 0), we do not allow multiple epsilon
							// transitions from state_s, as one could be on the path of fully epsilon run, while the
							// other would not be, so we would mark something as safe epsilon run, which would not be
							// safe
							continue;
						}
...
Nft mata::nft::insert_levels(
	...
	// Construct a vector of size equal to levels.num_of_levels. Each index k in this vector represents a new level for
	// the original level k. Note: The 0th index always remains zero. Here are a couple of examples to illustrate this:
	// Example 1:
	//   Input (levels)      : 0 1 2
	//   Mask                : 0 0 1 1 0 1
	...
	for (auto it = new_levels_mask.rbegin(); it != new_levels_mask.rend(); ++it, --i) {
		...
		if (!*it) {
			if (jump_mode != JumpMode::AppendDontCares) { next_inner_levels[i] = static_cast<Level>(i + 1); }
	...
	auto get_inner_state = [&](const State src, const Level inner_level, const bool is_inserted_level,
							   const bool is_old_level_processed) {
	...
	for (const auto& trans : nft.delta.transitions()) {
		...
		const auto stop_level = static_cast<Level>(
			(result.levels[trans.target] == 0) ? (new_levels_mask.size() - 1) : (result.levels[trans.target] - 1)
		);
...
Nft nft::invert_levels(const Nft& aut, const JumpMode jump_mode) {
	...
	for (State s = 0; s < num_of_states; ++s) {
		if (aut.levels[s] == 0) { renaming[s] = num_of_zero_states++; }
	}
	...
	Nft aut_inv = Nft::with_levels(
		{aut.levels.num_of_levels, num_of_zero_states, DEFAULT_LEVEL}, num_of_zero_states, std::move(new_initial),
		std::move(new_final), aut.alphabets
	);
		...
		for (const auto& [src, symbol, tgt] : path) {
			...
			if (const bool is_tgt_tail = tgt == tail; is_src_head && is_tgt_tail) {
				...
				if (jump_mode == JumpMode::AppendDontCares && (aut.levels.num_of_levels - aut.levels[src]) > 1) {
					const State aux_state =
						get_aux_state(src, tgt, static_cast<Level>(aut_inv.levels[renaming[src]] - 1));
...
bool Nft::is_in_lang(
	const Run& run, const bool match_prefix, const JumpMode jump_mode, const bool has_epsilon_cycles
) const {
	return is_in_lang_by_levels(mk_level_word_from_word(run.word), match_prefix, jump_mode, has_epsilon_cycles);
}

bool Nft::is_in_lang_by_levels(
	const std::vector<Word>& level_words,
	const bool match_prefix,
	const JumpMode jump_mode,
	const bool has_epsilon_cycles
) const {
...
Word Nft::mk_word_from_level_word(const std::vector<Word>& level_words) const {
	...
	for (const auto& level_word : level_words) {
		for (const auto symbol : level_word) { word.push_back(symbol); }
	}
	return word;
}
...
Nft mata::nft::minimize(const Nft& aut, const ParameterMap& params) {
	Nft result;
		...
		while (synchronized_iterator.advance()) {
			...
			if (target_res_exists) {
				...
				if (next_level != 0) {
					...
					targets_resolved = std::move(targets);
				}
			}
...
namespace {
/// Shared DFS traversal behind @c Nft::get_words(). Walks one path at a time and shares @p word as scratch space
...
void get_words_dfs_impl(
	...
	if (nft.final.contains(state)) { result.insert(word); }
	if (max_length.has_value() && depth >= max_length.value()) { return; }
	...
	for (const SymbolPost& symbol_post : nft.delta[state]) {
		...
		for (size_t target_level{0}; target_level < map_level_targets.size(); ++target_level) {
			...
			for (const State target : targets_for_level) {
				get_words_dfs_impl(nft, target, depth + 1, max_length, jump_mode, word, result);
			}
...
std::set<Word> Nft::get_words(const std::optional<size_t> max_length, const JumpMode jump_mode) const {
	std::set<Word> result;
	Word word;
	for (const State init_state : initial) {
		get_words_dfs_impl(*this, init_state, 0, max_length, jump_mode, word, result);
	}
	return result;
}
