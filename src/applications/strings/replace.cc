...
void add_end_marker_transitions_from_literal_states(
	...
	// Skip the last state-word pair for the whole literal as it must first apply the replacement before accepting the
	// end marker.
	for (auto state_word_pair_it{state_word_pairs.begin()}; state_word_pair_it + 1 < state_pair_end;
		 ++state_word_pair_it) {
...
void handle_last_symbol_replacement(
...
void add_replacement_transitions(
	...
	for (auto replacement_it{replacement.begin()}; replacement_it < replacement_end; ++replacement_it) {
		...
		if (replacement_it + 1 == replacement_end) {
			handle_last_symbol_replacement(
				end_marker, replace_mode, nft, alphabet_symbols, state_lvl1, *replacement_it
			);
		} else {
			state_lvl0 = nft.add_state_with_level(0);
...
void add_generic_literal_transitions(
	const Word& literal,
	const std::vector<std::pair<State, Word>>& state_word_pairs,
	Nft& nft,
	const utils::OrdVector<Symbol>& alphabet_symbols
		...
		for (Symbol symbol : alphabet_symbols) {
			if (symbol == literal_symbol) { // Handle transition to next subword init_state.
				...
				while (subword_next_symbol_it != subword_next_symbol_end) {
					if (const Word subsubword{subword_next_symbol_it, subword_next_symbol_end};
						is_subsequence(subsubword, literal)) {
				...
				for (; subword_next_symbol_it_from_begin < subword_next_symbol_it;
					...
					if (subword_next_symbol_it_from_begin + 1 == subword_next_symbol_it) {
						nft.delta.add(state_lvl1, *subword_next_symbol_it_from_begin, target_state);
					} else {
						state_lvl0 = nft.add_state_with_level(0);
...
/// Add transitions, optionally add @p source to @p dfa_generic_end_marker.final, and update @p labeling and @p
/// labeling_inv functions.
void process_source(
	const nfa::Nfa& regex,
	const utils::OrdVector<Symbol>& alphabet_symbols,
...
Nfa mata::applications::strings::replace::reluctant_nfa(Nfa nfa) {
	for (const State final : nfa.final) { nfa.delta.mutable_state_post(final).clear(); }
	return nfa;
}

Nft mata::applications::strings::replace::create_identity(const Alphabet* const alphabet, const size_t num_of_levels) {
...
Nft mata::applications::strings::replace::create_identity_with_single_symbol_replace(
	const Alphabet* const alphabet, const Symbol from_symbol, const Symbol replacement, const ReplaceMode replace_mode
) {
	return create_identity_with_single_symbol_replace(alphabet, from_symbol, Word{replacement}, replace_mode);
}

Nft applications::strings::replace::create_identity_with_single_symbol_replace(
	...
	switch (replace_mode) {
		case ReplaceMode::All: {
			nft.delta.add(
				state_lvl1, replacement_it == replacement_end ? EPSILON : *replacement_it, *nft.initial.begin()
			);
			break;
		}
		case ReplaceMode::Single: {
			...
			nft.delta.add(
				state_lvl1, replacement_it == replacement_end ? EPSILON : *replacement_it, after_replace_state
			);
...
Nft mata::applications::strings::replace::replace_reluctant_regex(
	...
	return replace_reluctant_regex(
		nfa::determinize(nfa::builder::create_from_regex(regex)), replacement, alphabet, replace_mode, begin_marker
	);
}
...
nfa::Nfa ReluctantReplace::generic_marker_dfa(const std::string& regex, const Alphabet* const alphabet) {
...
nfa::Nfa ReluctantReplace::generic_marker_dfa(nfa::Nfa regex, const Alphabet* const alphabet) {
	...
	while (!worklist.empty()) {
		...
		if (StateSet& source_label{labeling[source]}; regex.final.intersects_with(source_label)) {
			...
			process_source(
				regex, alphabet_symbols, dfa_generic_end_marker, labeling, labeling_inv, source, source_label, worklist
			);
		}
	}
...
nfa::Nfa ReluctantReplace::begin_marker_nfa(const std::string& regex, const Alphabet* const alphabet) {
	return begin_marker_nfa(nfa::determinize(nfa::builder::create_from_regex(regex)), alphabet);
}

nfa::Nfa ReluctantReplace::begin_marker_nfa(nfa::Nfa regex, const Alphabet* const alphabet) {
	regex = revert(regex);
...
Nft ReluctantReplace::reluctant_leftmost_nft(
	...
	return reluctant_leftmost_nft(
		nfa::determinize(nfa::builder::create_from_regex(regex)), alphabet, begin_marker, replacement, replace_mode
	);
}
...
Nft ReluctantReplace::reluctant_leftmost_nft(
	...
	Nft nft_reluctant_leftmost{nft::builder::from_nfa_with_levels_zero(nfa, 2, true, {EPSILON})};
	...
	nft_reluctant_leftmost.delta.mutable_state_post(curr_state)
		.push_back(SymbolPost{EPSILON, StateSet{nft_reluctant_leftmost.initial}});
...
Nft applications::strings::replace::replace_reluctant_literal(
	...
	const Word& replacement,
	const Alphabet* const alphabet,
	const applications::strings::replace::ReplaceMode replace_mode,
	const Symbol end_marker
) {
	return ReluctantReplace::replace_literal(literal, replacement, alphabet, replace_mode, end_marker);
}

Nft ReluctantReplace::replace_literal_nft(
	const Word& literal,
	const Word& replacement,
	const Alphabet* const alphabet,
	const Symbol end_marker,
...
Nft applications::strings::replace::replace_reluctant_single_symbol(
	const Symbol from_symbol, const Word& replacement, const Alphabet* const alphabet, const ReplaceMode replace_mode
) {
	return ReluctantReplace::replace_symbol(from_symbol, replacement, alphabet, replace_mode);
}

Nft applications::strings::replace::replace_reluctant_single_symbol(
	const Symbol from_symbol, const Symbol replacement, const Alphabet* const alphabet, const ReplaceMode replace_mode
) {
	return ReluctantReplace::replace_symbol(from_symbol, replacement, alphabet, replace_mode);
}

Nft ReluctantReplace::replace_regex(
	nfa::Nfa aut,
	const Word& replacement,
	const Alphabet* const alphabet,
	const ReplaceMode replace_mode,
	...
	if (aut.is_lang_empty()) { return create_identity(alphabet); }
...
Nft ReluctantReplace::replace_literal(
	const Word& literal,
	const Word& replacement,
	const Alphabet* const alphabet,
	const ReplaceMode replace_mode,
	...
	const Nft nft_literal_replace{replace_literal_nft(literal, replacement, alphabet, end_marker, replace_mode)};
