...
void Delta::copy_to_boost(bool max_states)
{
    for (auto state_post = state_posts_.begin(); state_post < state_posts_.end(); state_post++)
        (*state_post).copy_to_boost(max_states ? num_of_states() : Limits::max_state);
}

void StatePost::copy_to_boost(State num_of_states)
{
    for (auto symbol_post = begin(); symbol_post < end(); symbol_post++)
        (*symbol_post).copy_to_boost(num_of_states);
}

void SymbolPost::copy_to_boost(State num_of_states) {
    State new_size = num_of_states == Limits::max_state ? (*std::max_element(targets.begin(), targets.end())) + 1 : std::max(num_of_states, (*std::max_element(targets.begin(), targets.end())) + 1);
    targets_boost.resize(new_size);
    for(auto it = targets.begin(); it != targets.end(); it++)
    {
        targets_boost.set(*it, true);
    }
}

void Delta::print_targets_boost()
{
    int i = 0;
    for (auto state_post = state_posts_.begin(); state_post < state_posts_.end(); state_post++)
    {
        std::cout << "targets for state " << i++ << ":" << std::endl;
        (*state_post).print_targets_boost();
    }
    std::cout << std::endl;
}
...
void Delta::print_targets_normal()
{
    ...
    {
        std::cout << "targets for state " << i++ << ":" << std::endl;
        (*state_post).print_targets_normal();
    }
    std::cout << std::endl;
}
...
bool Delta::Transitions::const_iterator::operator==(const Delta::Transitions::const_iterator &other) const
{
    ...
    {
        return current_state_ == other.current_state_ && state_post_it_ == other.state_post_it_ && symbol_post_it_ == other.symbol_post_it_;
    }
}
...
bool StatePost::Moves::const_iterator::operator==(const StatePost::Moves::const_iterator &other) const
{
    ...
    return symbol_post_it_ == other.symbol_post_it_ && target_it_ == other.target_it_ && symbol_post_end_ == other.symbol_post_end_;
}
...
Symbol Delta::get_max_symbol() const
{
    ...
    for (const StatePost &state_post : state_posts_)
    {
        for (const SymbolPost &symbol_post : state_post)
        {
            if (symbol_post.symbol > max)
            {
                max = symbol_post.symbol;
            }
        }
    }
    return max;
...
mata::utils::BoostVector SynchronizedExistentialSymbolPostIterator::unify_targets_boost() const {
    ...
    for (const StatePost::const_iterator &symbol_post_it : get_current())
    {
        unified_targets |= symbol_post_it->targets_boost;
    }
...
bool SynchronizedExistentialSymbolPostIterator::synchronize_with(const Symbol sync_symbol)
{
    do
    {
        if (is_synchronized())
        {
            ...
            if (current_min_symbol_post_it->symbol >= sync_symbol)
            {
                break;
            }
        }
