...
namespace
{
    Simlib::Util::BinaryRelation compute_fw_direct_simulation(const Nfa &aut)
    {
        ...
        if (!used_symbols.empty() && *used_symbols.begin() == 0)
        {
            ...
            if (unused_symbol == 0)
            { // sanity check to see if we did not use the full range of mata::Symbol
    ...
    Nfa reduce_size_by_simulation(const Nfa &aut, StateRenaming &state_renaming)
    {
        ...
        for (State q = 0; q < num_of_states; ++q)
        {
            ...
            if (state_renaming.count(qReprState) == 0)
        ...
        for (State q = 0; q < num_of_states; ++q)
        {
            ...
            if (aut.initial[q])
            { // if a symmetric class contains initial state, then the whole class should be initial
            ...
            { // we process only transitions starting from the representative state, this is enough for simulation
                for (const auto &q_trans : aut.delta.state_post(q))
                {
                    ...
                    for (const State s : representatives_of_states_to)
                    {
                        ...
                        {
                            if (s != p && sim_relation.get(s, p))
                            {                               // if p (different from s) simulates s
                ...
                if (aut.final[q])
                { // if q is final, then all states in its class are final => we make q_class_state final
    ...
    void remove_covered_state(const StateSet &covering_set, const State remove, Nfa &nfa)
    {
        ...
        { // remove trans from covered state
        ...
        for (const auto &move : remove_transitions)
    ...
    Nfa residual_with(const Nfa &aut)
    { // modified algorithm of determinization
        ...
        while (!worklist.empty())
        {
            ...
            while (synchronized_iterator.advance())
            {
                ...
                { // add new state
                    ...
                    if (T != covering_states[Tid])
                    {                                                       // new state is not covered, replace transitions
                                    ...
                                    Nfa &nfa)
    {
        ...
        if (covering_set == check_state)
        { // can recurse even without covered :thinking:
    ...
    Nfa residual_after(const Nfa &aut)
    {
        ...
        for (const auto &pair : subset_map)
        { // order by size from largest to smallest
            macrostate_vec.insert(std::upper_bound(macrostate_vec.begin(), macrostate_vec.end(), pair.first,
                                                   [](const StateSet &a, const StateSet &b)
        ...
        for (size_t i = 0; i < macrostate_size - 1; i++)
        {
            ...
            for (size_t j = i + 1; j < macrostate_size; j++)
            {                   // find covering macrostates
                if (covered[j]) // if covered there are smaller macrostates, skip
                    continue;

                if (macrostate_vec[j].is_subset_of(macrostate_vec[i]))
                {                                           // found covering state
...
Nfa mata::nfa::remove_epsilon(const Nfa &aut, Symbol epsilon)
{
    ...
    for (const auto &state_closure_pair : eps_closure)
    { // For every state.
        State src_state = state_closure_pair.first;
        for (State eps_cl_state : state_closure_pair.second)
        { // For every state in its epsilon closure.
            if (aut.final[eps_cl_state])
                result.final.insert(src_state);
...
Nfa mata::nfa::somewhat_simple_revert(const Nfa &aut)
{
    ...
    for (State q = 0, states_num = result.delta.num_of_states(); q < states_num; ++q)
    {
        ...
        for (SymbolPost &m : result.delta.mutable_state_post(q))
        {
            sort_and_rmdupl(m.targets);
        }
    }
...
bool mata::nfa::Nfa::is_prfx_in_lang(const Run &run) const
{
    ...
    for (const Symbol sym : run.word)
    {
        if (this->final.intersects_with(current_post))
        {
            return true;
        }
        current_post = this->post(current_post, sym);
...
std::optional<mata::Word> Nfa::get_word(const Symbol first_epsilon) const
{
    ...
    if (initial.intersects_with(final))
    {
        return Word{};
    }
        ...
        while (!worklist.empty())
        {
            ...
            if (state_post_it != state_post_end)
            {
                ...
                { // targets_it == state_post_it->targets.cend().
                    ...
                    if (state_post_it != state_post_end)
                    {
                        targets_it = state_post_it->cbegin();
                    }
                }
            }
            else
    ...
    {
        if (symbol_post_it->symbol < first_epsilon)
        {
...
std::optional<mata::Word> Nfa::get_word_from_complement(const Alphabet *alphabet) const
{
    ...
    while (continue_complementation && !worklist.empty())
    {
        ...
        while (sync_it_advanced || symbols_it != symbols_end)
        {
            ...
            if (!continue_complementation)
            {
                break;
            }
            if (symbol_advanced_to >= *symbols_it)
            {
                ++symbols_it;
            }
        ...
        macrostate_discover)
{
    ...
    auto subset_macrostate_map_ptr{
        subset_macrostate_map.emplace(
                                 std::make_pair(subset_set_included_ptr, subset_set_excluded_ptr), new_initial)
            .first.operator->()};
    ...
    while (!worklist.empty())
    {
        ...
        while (sync_it_included_advanced)
        {
            ...
            if (macrostate_inserted)
            {
                ...
                {
                    if (subset_set_excluded_ptr == subset_set_excluded_sink_ptr || (sync_it_excluded_advanced && !final_excluded_intersects_targets))
                    {
