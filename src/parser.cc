...
bool is_string_char(const char ch) { // {{{
	return !std::isblank(ch) && !(haskey(std::set<char>{'"', '(', ')', '#', '%', '@', '\\'}, ch));
} // is_string_char }}}
	...
	while (input.good()) {
		...
		switch (state) {
			...
			case TokenizerState::Quoted: {
				if ('"' == ch) {
					ch = input.peek();
					if (!std::isspace(ch) && ('#' != ch) && (')' != ch) && std::char_traits<char>::eof() != ch) {
						std::string context;
						std::getline(input, context);
						throw std::runtime_error("misplaced quotes: \"" + result + "_\"_" + context);
					}
					...
					state = TokenizerState::QuotedEscape;
				} else {
					result += static_cast<char>(ch);
				}
				break;
	...
	while (input.good()) {
		...
		if (reading_type) { // we're expecting a @TYPE declaration
			...
			if ('#' == line[0] || '\n' == line[0]) {
				...
				throw std::runtime_error("expecting automaton type (@TYPE), got \"" + line + "\" instead");
			}
			...
			while (i < line.size()) {
				if (std::isspace(line[i])) {
					...
					throw std::runtime_error(
						"invalid trailing characters \"" + trailing + "\" on the line \"" + line + "\""
					);
				}
			}
		...
		if (!quoted && '%' == maybe_key[0]) {
			...
			if (result.dict.end() == it) { // insert an empty list
				tie(it, std::ignore) = result.dict.insert({key, std::vector<std::string>()});
			}

			std::vector<std::string>& val_list = it->second;
			std::transform(
				token_line.begin() + 1, token_line.end(), std::back_inserter(val_list),
				[](const std::pair<std::string, bool>& token) { return token.first; }
			);
		} else {
			BodyLine stripped_token_line;
			std::ranges::transform(
				token_line, std::back_inserter(stripped_token_line), [&](const std::pair<std::string, bool>& token) {
					if (keep_quotes && token.second) {
						return "\"" + token.first + "\"";
					} else {
						return token.first;
					}
...
ParsedSection mata::parser::parse_mf_section(const std::string& input, const bool keep_quotes) {
	std::istringstream stream(input);
