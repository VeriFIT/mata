...
TEST_CASE("mata::Parser basic_parsing") {
	...
	SECTION("Additional parenthesis") {
		...
		SECTION("No parenthesis") { aut = mata::parser::create_nfa("a*b"); }

		SECTION("Around example parenthesis") { aut = mata::parser::create_nfa("(a*b)"); }

		SECTION("Around variable 'a' parenthesis") { aut = mata::parser::create_nfa("(a)*b"); }

		SECTION("Around variable 'b' parenthesis") { aut = mata::parser::create_nfa("a*(b)"); }

		SECTION("Parenthesis after iteration") { aut = mata::parser::create_nfa("((a)*)b"); }

		SECTION("Double parenthesis around 'b'") { aut = mata::parser::create_nfa("(a*(b))"); }

		SECTION("Double parenthesis around 'a'") { aut = mata::parser::create_nfa("((a)*b)"); }

		SECTION("Many parenthesis") { aut = mata::parser::create_nfa("(((a)*)b)"); }

		SECTION("Double parenthesis") { aut = mata::parser::create_nfa("((a))*((b))"); }

		SECTION("Double parenthesis after iteration") { aut = mata::parser::create_nfa("((((a))*))((b))"); }

		SECTION("Many parenthesis with double parenthesis") { aut = mata::parser::create_nfa("(((((a))*))((b)))"); }
...
TEST_CASE("mata::Parser UTF-8 encoding") { // {{{
	SECTION("below 0x80") {
		Nfa x = mata::parser::create_nfa(
			"\\x{01}\\x{10}\\x{20}\\x{30}\\x{40}\\x{50}\\x{60}\\x{70}\\x{7f}", false, 306, true, Encoding::Utf8
		);
		CHECK(x.is_in_lang(Run{Word{0x01, 0x10, 0x20, 0x30, 0x40, 0x50, 0x60, 0x70, 0x7f}, {}}));
		Nfa y;
		y.initial.insert(0);
	...
	SECTION("between 0x80 and 0x800") {
		Nfa x = mata::parser::create_nfa(
			"\\x{80}\\x{90}\\x{a0}\\x{b0}\\x{c0}\\x{d0}\\x{600}\\x{700}\\x{7ff}", false, 306, true, Encoding::Utf8
		);
		CHECK(x.is_in_lang(
			Run{mata::encode_word_utf8(Word{0x80, 0x90, 0xa0, 0xb0, 0xc0, 0xd0, 0x6'00, 0x7'00, 0x7'ff}), {}}
		));
		Nfa y;
		y.initial.insert(0);
	...
	SECTION("between 0x800 and 0x7FFF") {
		Nfa x = mata::parser::create_nfa(
			"\\x{800}\\x{900}\\x{a00}\\x{b00}\\x{c00}\\x{d00}\\x{6000}\\x{7000}\\x{7fff}", false, 306, true,
			Encoding::Utf8
		);
		CHECK(x.is_in_lang(
			Run{mata::encode_word_utf8(Word{0x8'00, 0x9'00, 0xa'00, 0xb'00, 0xc'00, 0xd'00, 0x60'00, 0x70'00, 0x7f'ff}),
				{}}
		));
		Nfa y;
		y.initial.insert(0);
	...
	SECTION("between 0x10000 and 0x10FFFF") {
		Nfa x = mata::parser::create_nfa(
			"\\x{10000}\\x{20000}\\x{30000}\\x{40000}\\x{50000}\\x{60000}\\x{70000}\\x{80000}\\x{10ffff}", false, 306,
			true, Encoding::Utf8
		);
		CHECK(x.is_in_lang(
			Run{mata::encode_word_utf8(
					Word{
						0x1'00'00, 0x2'00'00, 0x3'00'00, 0x4'00'00, 0x5'00'00, 0x6'00'00, 0x7'00'00, 0x8'00'00,
						0x10'FF'FF
					}
				),
				{}}
	...
	SECTION("Regex range [x70-x90]") {
		...
		for (Symbol c = 0x70; c <= 0x90; c++) { result.delta.add(initial_s, c, final_s); }
	...
	SECTION("Regex range [x790-x890]") {
		...
		result.initial.insert(initial_s);
		result.final.insert(final_s);
		for (Symbol c = 0x7'00; c <= 0x9'00; c++) { result.delta.add(initial_s, c, final_s); }
		CHECK(are_equivalent(aut, result));
	}
	...
	SECTION("Regex range [xFF90-x10090]") {
		...
		result.initial.insert(initial_s);
		result.final.insert(final_s);
		for (Symbol c = 0xFF'90; c <= 0x1'00'90; c++) { result.delta.add(initial_s, c, final_s); }
		CHECK(are_equivalent(aut, result));
	}
	...
	SECTION("Regex [\\x{00}-\\x{10FFFF}]") {
		...
		// Random symbols
		std::vector<Symbol> symbols = {0x00,	   0x10,	   0x20,	   0x30,	   0x40,	   0x50,	  0x60,
									   0x70,	   0x7f,	   0x80,	   0x90,	   0xa0,	   0xb0,	  0xc0,
									   0xd0,	   0xe0,	   0xf0,	   0xff,	   0x1'00,	   0x1'10,	  0x36'f0,
									   0x57'fc,	   0x61'77,	   0x74'98,	   0x8f'3f,	   0x9f'c8,	   0x1'10'1e, 0x1'43'48,
									   0x1'4e'34,  0x1'95'81,  0x1'c4'8e,  0x1'f1'cc,  0x1'f9'1d,  0x2'22'a6, 0x2'2e'11,
									   0xe'54'f5,  0xe'79'34,  0xe'93'a4,  0xe'99'8d,  0xe'be'e8,  0xe'db'9e, 0xe'f9'8b,
									   0xf'12'af,  0xf'51'e2,  0xf'55'7f,  0xf'6b'08,  0xf'a7'f0,  0xf'ac'b2, 0xf'd7'19,
									   0x10'6d'12, 0x10'6d'66, 0x10'92'20, 0x10'a6'08, 0x10'c1'f5, 0x10'FF'FF};
...
TEST_CASE("Foldcase") {
	...
	SECTION("Regex [a-z]") {
		...
		result.initial.insert(initial_s);
		result.final.insert(final_s);
		for (Symbol c = 'a'; c <= 'z'; c++) { result.delta.add(initial_s, c, final_s); }
	...
	SECTION("Regex [M-Ya-x]") {
		...
		result.initial.insert(initial_s);
		result.final.insert(final_s);
		for (Symbol c = 'a'; c <= 'x'; c++) { result.delta.add(initial_s, c, final_s); }
		for (Symbol c = 'M'; c <= 'Y'; c++) { result.delta.add(initial_s, c, final_s); }
		CHECK(are_equivalent(nfa, result));
	}
	...
	SECTION("Regex [A-Ma-m]") {
		...
		result.initial.insert(initial_s);
		result.final.insert(final_s);
		for (Symbol c = 'A'; c <= 'M'; c++) { result.delta.add(initial_s, c, final_s); }
		for (Symbol c = 'a'; c <= 'm'; c++) { result.delta.add(initial_s, c, final_s); }
		CHECK(are_equivalent(nfa, result));
	}
